...
''' 
Adapted from DDRM (https://ddrm-ml.github.io/)
...
Original file: https://github.com/bahjat-kawar/ddrm/blob/master/functions/denoising.py
Credit to Bahjat Kawar (https://github.com/bahjat-kawar)
'''


def generation_steps(x, seq, model, betas, H_funcs, y_0, sigma_0, etaB, etaA, etaC, cls_fn=None, classes=None):
    with torch.no_grad():
        ...
        t = (torch.ones(x.size(0)) * seq[-1]).to(x.device).long()
        largest_alphas = (1 - betas).cumprod(dim=0).index_select(0, t + 1).view(-1, 1, 1, 1)
        ...
        for i, j in tqdm(zip(reversed(seq), reversed(seq_next))):
            ...
            at = (1 - betas).cumprod(dim=0).index_select(0, t.long() + 1).view(-1, 1, 1, 1)
            at_next = (1 - betas).cumprod(dim=0).index_select(0, next_t.long() + 1).view(-1, 1, 1, 1)
            xt = xs[-1].to('cuda')
            if cls_fn == None:
                et = model(xt, t)
            else:

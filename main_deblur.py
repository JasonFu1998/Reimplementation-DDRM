...
from utils.utils_svd import Deblurring
...
def main():
    ...
    testset_name            = 'imagenet_val'                  # 'ffhq_test' | 'imagenet_val' | 'ffhq_val' | 'imagenet_test'
    ...
    blur_mode               = 'Gaussian'        # Gaussian | Uniform      
    ...
    result_name             = f'{testset_name}_{blur_mode}_deblur_{generate_mode}_{model_name}_sigma{sigma_0}_NFE{iter_num}'
    ...
    def running():
        ...
        for idx, img in enumerate(L_paths):
            ...
            # Gaussian blur
            pdf = lambda x: torch.exp(torch.Tensor([-0.5 * (x / 10) ** 2]))
            kernel = torch.Tensor([pdf(-2), pdf(-1), pdf(0), pdf(1), pdf(2)]).to(device)
            H_funcs = Deblurring(kernel / kernel.sum(), n_channels, img_size, device)
            # Uniform blur
            # H_funcs = Deblurring(torch.Tensor([1/9] * 9).to(device), n_channels, img_size, device)
            
            y_0 = H_funcs.H(y)
            y_0 = y_0 + sigma_0 * torch.randn_like(y_0)
            
            pinv_y_0 = y_0.view(y_0.shape[0], n_channels, img_size, img_size)

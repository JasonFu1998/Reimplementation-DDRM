...
from utils.utils_svd import Inpainting
...
def main():
    ...
    model_name              = 'ffhq_diffusion'             # ffhq_diffusion | 256x256_diffusion_uncond
    testset_name            = 'ffhq_test'                  # 'ffhq_test' | 'imagenet_val' | 'ffhq_val' | 'imagenet_test'
    ...
    generate_mode           = 'DDRM'
    mask                    = 'our_mask'          # lolcat_extra | our_mask
    ...
    result_name             = f'{testset_name}_inp_{mask}_{generate_mode}_{model_name}_sigma{sigma_0}_NFE{iter_num}'
    ...
    def running():
        ...
        for idx, img in enumerate(L_paths):
            ...
            mask_n = np.expand_dims(loaded, axis=-1)
            mask_n = np.repeat(mask_n, 3, axis=-1)

            img_L = img_H * mask_n

            mask_tensor = torch.from_numpy(loaded).to(device).view(-1)
            miss_position = torch.nonzero(mask_tensor == 0).view(-1)
            miss = torch.cat([3 * miss_position, 3 * miss_position + 1, 3 * miss_position + 2], dim=0)

            H_funcs = Inpainting(n_channels, img_size, miss, device)
            ...
            if save_L:
                utils_function.imsave(img_L, os.path.join(E_path, img_name + '_L' + ext))
